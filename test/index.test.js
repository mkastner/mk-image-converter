...
async function main() {
    ...
    try {
      for (let key in fileTypes) {
        ...
        t.equal(
          mimeType,
          fileTypes[key],
          `mime type for ${key}: ${fileTypes[key]}`
        );
  ...
  tape('save base64', async (t) => {
    try {
      for (let key in fileTypes) {
        ...
        const saveResult = await assets.saveTempBase64(
          'base64-example',
          base64Data
        );
        ...
        const statResult = await gracefulStat(
          path.join(__dirname, 'results', 'tmp', `base64-example.${key}`),
          true
        );
  ...
  tape('save binary', async (t) => {
    try {
      for (let key in fileTypes) {
        ...
        const assets = await ImageConverter(path.join(__dirname, 'results'));
        const saveResult = await assets.saveTempBinary(
          `example.${key}`,
          exampleFile
        );
        ...
        t.equal(
          saveResult.mimeType,
          fileTypes[key],
          `returning mime type ${key[fileTypes]} for ${key}`
        );

        const statResult = await gracefulStat(
          path.join(__dirname, 'results', 'tmp', `example.${key}`),
          true
        );

        t.ok(statResult, `original file type "${key}" saved`);

        const assetsWithId = await ImageConverter(
          path.join(__dirname, 'results')
        );
        const result = await assetsWithId.saveTempBinary(
          `example.${key}`,
          exampleFile
        );
        ...
        t.ok(
          statResultWithId,
          `original file saved with subPath for file type ${key}`
        );
      }
    } catch (err) {
      log.error(err);
  ...
  tape('convert from original to different sizes and file types', async (t) => {
    try {
      ...
      async function testConvertType(fromFileName, convertArgs) {
        ...
        for (let i = 0, l = convertArgs.length; i < l; i++) {
          ...
          const statResultOriginalWithId = await gracefulStat(
            path.join(
              ...
              'results',
              convertArg.type,
              `example.${convertArg.ext}`
            )
          );
          t.ok(
            statResultOriginalWithId,
            `${convertArg.type} version must be removed`
          );
        }
        ...
        const availableFiles = await assetsWithId.convert(
          fromFileName,
          convertArgs,
          { subPath }
        );
    ...
    try {
      for (let key in fileTypes) {
        ...
        for (let i = 0, l = convertArgs.length; i < l; i++) {
          ...
          t.notOk(
            statResultOriginalWithId,
            `${convertArg.type} version for ${key} must be removed`
          );
        }
        ...
        t.notOk(
          statResultOriginalWithId,
          `Original version for ${key} must be removed`
        );
      }
    } catch (err) {
      log.error(err);
  ...
  tape('check utility method tempfileExists', async (t) => {
    try {
      const base64PNGData = await fs.readFile(
        path.join(__dirname, 'assets', 'example-png.base64'),
        'utf8'
      );
      const assets = await ImageConverter(path.join(__dirname, 'results'));
  ...
  tape('convert for visual checking', async (t) => {
    try {
      ...
      const assets = await ImageConverter(
        path.join(__dirname, 'visual-results')
      );
